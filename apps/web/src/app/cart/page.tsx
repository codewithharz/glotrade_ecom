...
export default function CartPage() {
        ...
        try {
          ...
          if (raw) {
            ...
            if (user?.id || user?._id) {
              ...
              const addressRes = await fetch(new URL(`/api/v1/users/me/addresses`, API_BASE_URL).toString(), {
                headers: { Authorization: `Bearer ${user.id || user._id}` },
                cache: "no-store"
              });

              console.log('Cart: Address API response status:', addressRes.status);
  ...
  return (
    <RequireAuth cancelRedirect="/" message="Please sign in to view your cart.">
      <main className="mx-auto md:w-[95%] w-full px-2 md:px-6 py-4 sm:py-5">
        <nav className="mb-3 text-sm text-neutral-500"><Link href="/">{translate(locale, "home")}</Link> <span className="mx-2">›</span> <span className="text-neutral-700">{translate(locale, "cartTitle")}</span></nav>
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-6">
          <section>
            <div className="rounded border border-emerald-200 bg-emerald-50 dark:border-emerald-800 dark:bg-emerald-900/20 p-2 text-sm flex items-center justify-between">
              <div className="inline-flex items-center gap-2 text-semibold text-emerald-700 dark:text-emerald-300 text-[10px] sm:text-sm"><Check size={16} /> {translate(locale, "freeShippingBanner")}</div>
              <div className="text-grey-500 font-semibold text-[10px] sm:text-sm">{translate(locale, "limitedTimeOffer")}</div>
            </div>

            <div className="mt-3 flex items-center gap-3 text-sm">
              <label className="inline-flex items-center gap-2 cursor-pointer">
                <input type="checkbox" checked={allSelected} onChange={toggleAll} />
                <span>{translate(locale, "selectAll")} ({Object.keys(idToQty).length})</span>
              </label>
            </div>
            ...
            <div className="mt-3 space-y-3">
              {loading ? (
                ...
                <div className="text-sm text-neutral-500">{translate(locale, "yourCartIsEmpty")}</div>
              ) : (
          ...
          <aside className="lg:sticky lg:top-20 h-max">
            <div className="rounded-lg border border-neutral-200 dark:border-neutral-800 p-4 sm:p-5">
              <h3 className="text-base sm:text-lg font-semibold mb-3 sm:mb-4">{translate(locale, "orderSummary")}</h3>
              <div className="space-y-2 sm:space-y-1">
                <div className="flex items-center justify-between text-sm py-1"><span>{translate(locale, "itemsTotal")}</span><span className="line-through">{summary.subtotal.toLocaleString()} NGN</span></div>
                <div className="flex items-center justify-between text-sm py-1"><span>{translate(locale, "itemsDiscount")}</span><span className="text-rose-600">-{summary.discount.toLocaleString()} NGN</span></div>
                ...
                <div className="flex items-center justify-between font-semibold text-base sm:text-lg py-1"><span>{translate(locale, "total")}</span><span className="text-orange-600">{summary.total.toLocaleString()} NGN</span></div>
              </div>
              <div className="mt-3 text-xs text-neutral-500">{translate(locale, "youWillNotBeCharged")}</div>
              <button
              ...
              >
                {translate(locale, "checkout")} ({summary.items})
              </button>

              <div className="mt-5 space-y-3 sm:space-y-4 text-xs sm:text-sm">
                <div className="font-semibold text-sm sm:text-base">{translate(locale, "You can checkout as a guest.")}</div>

                <div>
                  <div className="flex items-center gap-2 font-semibold text-sm sm:text-base"><ShieldCheck size={16} /> {translate(locale, "safePaymentOptions")}</div>
                ...
                <div>
                  <div className="font-semibold text-sm sm:text-base">{translate(locale, "securityCertification")}</div>
                  <div className="mt-2 grid grid-cols-3 sm:grid-cols-3 gap-1.5 sm:gap-2 text-[10px] sm:text-[11px] text-neutral-700">
                ...
                <div>
                  <div className="flex items-center gap-2 font-semibold text-sm sm:text-base"><Lock size={16} /> {translate(locale, "securePrivacy")}</div>
                  <p className="mt-1 text-[10px] sm:text-xs text-neutral-500 leading-relaxed">{translate(locale, "We protect your privacy and keep your information secured and uncompromised. We will only use your information in accordance with our privacy policy to provide and improve our services to you.")}</p>
                  <Link href="#" className="text-[10px] sm:text-xs text-blue-600 hover:underline inline-flex items-center gap-1 mt-1">{translate(locale, "learnMore")}</Link>
                </div>

                <div>
                  <div className="flex items-center gap-2 font-semibold text-sm sm:text-base"><ShieldCheck size={16} /> {translate(locale, "purchaseProtection")}</div>
                  <p className="mt-1 text-[10px] sm:text-xs text-neutral-500">{translate(locale, "Shop confidently knowing that if something goes wrong, we've always got your back.")}</p>
                  <Link href="#" className="text-[10px] sm:text-xs text-blue-600 hover:underline inline-flex items-center gap-1">{translate(locale, "seeProgramTerms")}</Link>
                </div>

                <div className="flex items-center gap-2 font-semibold text-sm sm:text-base"><Leaf size={16} /> {translate(locale, "treePlantingProgram")}</div>
              </div>
            </div>
          </aside>
        </div>
        ...
        <AddressModal
          ...
          onSaved={() => {
            ...
            setToastMsg(translate(locale, "Address saved successfully. You can proceed to checkout."));
