...
export default function BulkPricingTiers({ productId, price, currency, tiers }: Props) {
                ...
                <div className="space-y-2">
                    {tiers.map((tier, idx) => {
                        ...
                        if (tier.pricePerUnit !== undefined && tier.pricePerUnit >= 0) {
                            ...
                            priceDisplay = `${currency} ${tier.pricePerUnit.toFixed(2)} each`;
                            savingsDisplay = `Save ${savingsPercent}%`;
                        } else if (tier.discountPercent !== undefined && tier.discountPercent > 0) {
                            const effectivePrice = price * (1 - tier.discountPercent / 100);
                            priceDisplay = `${currency} ${effectivePrice.toFixed(2)} each`;
                            savingsDisplay = `${tier.discountPercent}% off`;
                        }
                        ...
                        return (
                            ...
                            >
                                <span className="font-medium text-blue-900 dark:text-blue-100">{quantityRange} units</span>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4" onClick={() => setSelectedTier(null)}>
                    <div className="bg-white dark:bg-neutral-800 rounded-xl shadow-xl max-w-sm w-full p-6 animate-in fade-in zoom-in duration-200" onClick={e => e.stopPropagation()}>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">
                            Do you want to add <strong>{selectedTier.minQuantity} units</strong> to your cart to unlock this wholesale price?
                        </p>
