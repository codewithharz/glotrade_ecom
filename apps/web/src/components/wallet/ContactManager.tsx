...
export default function ContactManager({ onSelectContact, onClose }: ContactManagerProps) {
  ...
  const deleteContact = async (contactId: string) => {
    if (!confirm(translate("wallet.contacts.deleteConfirm"))) return;
        ...
        <div className="flex items-center justify-between p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 flex-shrink-0">
          <div className="min-w-0 flex-1">
            <h3 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white truncate">
              {translate("wallet.contacts.title")}
            </h3>
            <p className="text-xs sm:text-sm text-gray-500 dark:text-gray-400">
              {translate(filteredContacts.length === 1 ? "wallet.contacts.count" : "wallet.contacts.count_plural", { count: filteredContacts.length })}
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-4 sm:p-6 border-b border-gray-200 dark:border-gray-700 space-y-3 sm:space-y-4 flex-shrink-0">
          <div className="flex gap-2 sm:gap-3">
            <div className="flex-1 relative">
              ...
              <input
                type="text"
                placeholder={translate("wallet.contacts.searchPlaceholder")}
                value={searchQuery}
            ...
            <button
              ...
              className={`px-3 sm:px-4 py-2 rounded-lg transition-colors ${showFavoritesOnly
                ? 'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-400'
                : 'bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-300'
                }`}
              title={showFavoritesOnly ? translate("wallet.contacts.allContacts") : translate("wallet.contacts.favoritesOnly")}
            >
          ...
          <div className="flex gap-2">
            ...
            >
              <option value="lastTransferred">{translate("wallet.contacts.sortOptions.lastTransferred")}</option>
              <option value="name">{translate("wallet.contacts.sortOptions.name")}</option>
              <option value="totalTransferred">{translate("wallet.contacts.sortOptions.totalTransferred")}</option>
              <option value="transferCount">{translate("wallet.contacts.sortOptions.transferCount")}</option>
            </select>
            <button
              ...
              title={sortOrder === 'asc' ? translate("wallet.contacts.sortDesc") : translate("wallet.contacts.sortAsc")}
            >
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-6">
          {filteredContacts.length === 0 ? (
            <div className="text-center py-6 sm:py-8">
              ...
              <p className="text-gray-500 dark:text-gray-400 mb-2 text-sm sm:text-base">{translate("wallet.contacts.noContacts")}</p>
              <p className="text-xs sm:text-sm text-gray-400 dark:text-gray-500">
                {searchQuery ? translate("wallet.contacts.adjustSearch") : translate("wallet.contacts.addHint")}
              </p>
            </div>
          ) : (
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        <button
                          ...
                          title={contact.isFavorite ? translate("wallet.contacts.unfavorite") : translate("wallet.contacts.favorite")}
                        >
                          {contact.isFavorite ? (
                        ...
                        <button
                          ...
                          className="p-2 hover:bg-gray-200 dark:hover:bg-gray-600 rounded-lg transition-colors"
                          title={translate("wallet.contacts.edit")}
                        >
                        ...
                        >
                          {translate("wallet.contacts.send")}
                        </button>
                      </div>
                    </div>
                  </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-xl max-w-md w-full p-4 sm:p-6 shadow-xl max-h-[90vh] overflow-y-auto">
              <div className="flex items-center justify-between mb-4">
                <h4 className="text-base sm:text-lg font-semibold text-gray-900 dark:text-white">
                  {translate("wallet.contacts.editTitle")}
                </h4>
                <button
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    {translate("wallet.contacts.notes")}
                  </label>
                  <textarea
                    value={editNotes}
                    onChange={(e) => setEditNotes(e.target.value)}
                    placeholder={translate("wallet.contacts.notesPlaceholder")}
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">
                    {translate("wallet.contacts.tags")}
                  </label>
                  <div className="flex gap-2 mb-2">
                    <input
                      ...
                      placeholder={translate("wallet.contacts.tagPlaceholder")}
              ...
              <div className="flex gap-2 sm:gap-3 mt-6">
                ...
                >
                  {translate("wallet.contacts.saveChanges")}
                </button>
                <button
                  onClick={() => setEditingContact(null)}
                ...
                >
                  {translate("wallet.contacts.cancel")}
                </button>
              </div>
            </div>
          </div>
